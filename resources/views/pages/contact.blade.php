        ...
        <div class="container">
            @foreach($contacts as $contact)
                <div class="process_heading w-100 text-center aos-init aos-animate" data-aos="fade-down">
                    <span
                        class="fs-ten fw-semibold p2-color mb-2 text-center">{{ $contact->getTranslatedAttribute('subtitle', session('locale')) }}</span>
                    <h2 class="fs-two fw-semibold p8-color mb-3 mb-lg-6">
                        {{ $contact->getTranslatedAttribute('title', session('locale')) }}
                    </h2>
            ...
            <div class="row g-3 g-md-6 mt-5 mt-md-10">
                <div class="col-lg-8">
                    <form action="{{ url('/' . app()->getLocale() . '/contact') }}" id="contact-form"
                        class="bg14-color py-5 py-md-10 px-4 px-md-8 border cus-border border-six rounded-4" method="post">
                        @csrf
                        ...
                        {{-- <div class="mb-4 mb-md-8">--}}
                            ...
                            {{-- <input type="text"
                                class="p4-color border cus-border border-six px-3 px-md-6 py-2 py-md-4 rounded"
                                placeholder="Write your Subject" id="subject" required="">--}}
                            {{-- </div>--}}
                        <div class="mb-5 mb-md-10">
                            ...
                            <textarea name="message"
                                ...
                                placeholder="ERP system" id="message"></textarea>
                        </div>
                    ...
                    <div class="bg1-color px-5 px-lg-10 py-8 py-md-15 rounded-4">
                        @foreach($contactInfos as $contactInfo)
                            <div class="mb-5 mb-md-6 mb-xxl-11">
                                <h4 class="fs-five p6-color mb-2">
                                    {{ $contactInfo->getTranslatedAttribute('type', session('locale')) }}</h4>
                                @if($contactInfo->icon)
                                    <a href="{{ $contactInfo->url }}" target="_blank"
                                        class="contact_icon d-flex justify-content-start gap-2 align-items-center">
